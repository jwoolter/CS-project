...
npcs = {"Guy":("Greetings", "I am"), "Aldith":("Hello", "I am"), "Rowan":("Good morning","I am"), "Borin":("Hi","I am"), "Piere":("Bonjour", "je suis"), "Dora":("Hola", "me llamo"), "Emperor Chu":("Ni hao","Wo jao"), "not important":("Ah", "my name is"), "Aleixo-Defender of man":("Olá", "meu nome é")}
...
def monkey_forest(name, npc, npc2):
  ...
  answer= scene(npc + " vanishes and you find yourself before an archway leading to a forest", "Will you enter?",["yes", "no"])
  if answer == "yes":
   myprint("you slowly walk into the forest.")
    ...
    x=input()
    answer = scene("As you walk along the dirt leading away from the forest you notice a monkey engulfed by shadow emerge infront of you.", "How do you react?",["Run back to the forest", "fight the monkey head on!"])
  ...
  answer = scene("whilst you are walking through the forest 5 monkeys drop down from the trees", "how can you get past", ["fight the monkeys","carry on running through the forest", "surrender to the monkeys"])
  ...
  if health <=0:
    ...
    x=input()
    answer = scene("At the end of the clearing you see a torch with a white flame and an eternal bonfire", "Do you light it?", ["yes", "no"])
    if answer == "yes":
      fires_lit += 1
